...
describe('/articles', function(){
    ...
    describe('Find', function(){
        ...
        //     agent
        //     .get('/articles/'+oneId2+'?populate=toPop')
        //     .set('x-tag',tag)
        //     .then(function(resp){
        ...
        it('should load next page for pagination', function(done){
            agent
            .get('/articles?lastId='+last+'')
            .set('x-tag',tag)
            .expect(200,done);
        });
        it('should filter by date range', function(done){
            agent
            .get('/articles?from='+from+'&to='+new Date().toISOString())
            .set('x-tag',tag)
            .then(function(resp){
                resp.body.data.length.should.be.above(0);
                done();
...
describe('Restore', function(){
    it('should restore a previously deleted data', function(done){
        ...
        setTimeout(function(){
            ...
            .then(function(resp){
                return agent
                .post('/articles/'+resp._id+'/restore')
                .set('x-tag',tag);
            })
            .then(function(resp){
                resp.body.data.should.be.an('object');
