...
var debug = require('debug')('articlesController');
...
ArticlesController.buildProjection = function(projections){
...
ArticlesController.find = function(req,res,next){
    var query;
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = ArticlesController.buildProjection(projection);
            delete query.select;
        }
        ...
        _query.where = query;
        var totalResult = Articles.count(_query);
        ...
        if(projection){
            ...
            .spread(function(resp,total,totalResult){
                ...
                return [Articles.findAll(_query),total,totalResult];
            })
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
            ...
            q.all([Articles.findAll(_query),total,totalResult])
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
                    ourLastId = null;
...
ArticlesController.findOne = function(req,res,next){  
    var _query = {};
    ...
    _query.where = Articles.where = {_id:id};
    ...
    var question = Articles.findOne(_query);
...
ArticlesController.create = function(req,res,next){
    var data  = req.body;
    if(data && data.secure){
        delete data.secure;
    }
    ...
    var question;
    if(data.length){
        question = Articles.bulkCreate(data);
    }else{
        question = Articles.create(data);
    }
...
ArticlesController.update = function(req,res,next){
    var query = req.query;
    var _query = {};
    // Clean appId and userId
    if(query && query.appId){
...
ArticlesController.updateOne = function(req,res,next){
    var _query = {};
    var id = req.params.id;
    var data  = req.body;
    if(data && data.secure){
...
ArticlesController.delete = function(req,res,next){ 
    var _query = {};
    var query = req.query;
    // Clean appId and userId
    if(query && query.appId){
...
ArticlesController.deleteOne = function(req,res,next){
    var _query = {};
    var id = req.params.id;

    _query.where = {_id:id};
    // Find match
    Articles.findOne(_query)
    .then(function(resp){
        if(!resp){
...
ArticlesController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Articles.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
