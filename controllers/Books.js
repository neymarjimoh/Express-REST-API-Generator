...
var debug = require('debug')('booksController');

var service = 'Books';

var BooksController = {};

BooksController.buildProjection = function(projections){
...
BooksController.find = function(req,res,next){
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = BooksController.buildProjection(projection);
            delete query.select;
        }
        ...
        var totalResult = Books.estimatedDocumentCount(query);
        var total = Books.estimatedDocumentCount({});
        var question = Books.find(query);
...
BooksController.findOne = function(req,res,next){
    ...
    var question = Books.findById(id);
    if(populate){
        delete query.populate;
        question = question.populate(populate);
    }
...
BooksController.create = function(req,res,next){
    var data  = req.body;
    if(data && data.secure){
        delete data.secure;
    }
...
BooksController.updateOne = function(req,res,next){
    var id = req.params.id;
    var data  = req.body;
    if(data && data.secure){
        delete data.secure;
...
BooksController.delete = function(req,res,next){
    ...
    .then(function(resp){
        // Delete matches
        return [Books.deleteMany(query), resp];
    })
...
BooksController.deleteOne = function(req,res,next){
    var id = req.params.id;
    // Find match
    Books.findById(id)
    .then(function(resp){
    ...
    .then(function(resp){
        // Delete match
        return [Books.findByIdAndRemove(id),resp];
    })
    .spread(function(deleted,resp){
        if(!resp){
            next();
...
BooksController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Books.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
